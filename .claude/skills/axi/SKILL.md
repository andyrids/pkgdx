...
description: >-
  ...
  from memory. Also use when setting up, refreshing or troubleshooting the `pkgdx axi` CLI or
...
`pkgdx` is not on `PATH` here - it is a console script inside the project venv, so every
...
The bare `pkgdx axi ...` spelling used in `AGENTS.md` and in the `help[]` footers assumes an
...
help[1]:
  Run `pkgdx axi inspect <qualified_name>` for complete metadata
```

```sh
# 3. Inspect (add --docstring for the complete body, not just the first line)
uv run pkgdx axi inspect rich.console::Console.print --docstring
```
...
Verified against `pkgdx axi --help` output; defaults shown in parentheses.

| Command | Flags | Purpose |
| --- | --- | --- |
| `axi` | - | Home view: bin/venv paths, active status, next-step hints |
| `axi list` | `--all`, `--fields` (`name,version`) | Declared, installed venv packages |
| `axi show <pkg>` | `--fields` (`name,version,location`) | Installed package metadata |
| `axi show <pkg> --api` | `--docstring`, `--refresh` | Public top-level API symbols |
| `axi find <query>` | `--package`, `--limit` (`20`), `--refresh` | Search cached symbols |
| `axi tree <pkg>` | `--max-depth` (`2`), `--refresh` | Nested module tree |
| `axi inspect <name>` | `--docstring`, `--refresh` | Symbol detail, or module children |
| `axi inherits <qname>` | `--refresh` | Classes directly subclassing a base |
| `axi serve` | - | Run the MCP server over stdio |
| `axi setup` | - | Install ambient context (AGENTS.md + MCP config) |
...
`axi serve` exposes the same surface over stdio MCP under the server name `VenvAXI`. Tool
names are the camelCased form of the underlying functions in `src/pkgdx/axi/_mcp.py`. Every
tool returns a TOON string with the same `count:`/`help[]` contract as the CLI, and
`AXIError`s come back as a TOON error block rather than an MCP transport error.

| Tool | Parameters | CLI equivalent |
| --- | --- | --- |
| `listPackagesTool` | `include_dev=False` | `axi list [--all]` |
| `showPackageTool` | `name` | `axi show <pkg>` |
| `showPackageApiTool` | `name`, `docstring=False` | `axi show <pkg> --api` |
| `showModuleTool` | `name`, `docstring=False` | `axi inspect <module>` |
| `getSymbolTool` | `qualified_name`, `docstring=False` | `axi inspect <qname>` |
| `findSymbolTool` | `query`, `limit=20`, `package=None` | `axi find <query>` |
| `getInheritorsTool` | `qualified_name` | `axi inherits <qname>` |
| `getModuleTreeTool` | `name`, `max_depth=2` | `axi tree <pkg>` |
...
- **No tool takes a `refresh` parameter.** A stale graph can only be rebuilt from the CLI, so
  after a dependency version bump run `uv run pkgdx axi <cmd> ... --refresh` once, then carry
  on over MCP.
...
- **When to `--refresh`.** The cache lives at `~/.pkgdx/axi/<project-hash>.db` and already
...
- **MCP needs the extra.** `serve` requires `fastmcp` (`uv add pkgdx[axi]`) and exits `1`
  with a "requires the `pkgdx[axi]` extra" log line without it. `setup` deliberately *omits*
...
- **`setup` writes files - it is not a diagnostic command.** It rewrites `AGENTS.md`'s ambient
  ...
  answer it with `axi show fastmcp` (raises `PackageNotFoundError` if absent) rather than
...
- **Token savings are payload-shaped, not a flat ~40%.** Measured against compact JSON
  (`tests/test_axi_toon_benchmark.py`): `axi list` ~45%, `axi find` ~27%, `axi inspect
...
- `uv run pkgdx axi <cmd> --help` is the authoritative flag source. If the table above ever
...
- `ICM/_config/reference-standard-axi.md` covers the 10 AXI design principles, the measured
  ...
  modifying `src/pkgdx/axi/` itself; it is not needed to *use* the CLI.
- The always-on summary injected into `AGENTS.md` is generated from
  `src/pkgdx/axi/_ambient.py::_BLOCK_BODY` and refreshed by `pkgdx axi setup`. Edit the
