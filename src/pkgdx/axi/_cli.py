...
from pkgdx.axi._ambient import mcp_available, setup_ambient_context
from pkgdx.axi._introspect import (
    SYMBOL_INFO_FIELDS,
    find_symbol,
    get_inheritors,
    get_module_tree,
...
from pkgdx.axi._packages import (
    PACKAGE_INFO_FIELDS,
    list_packages,
    resolve_package,
)
from pkgdx.axi._toon import encode_object, encode_table, format_help
from pkgdx.exceptions import InvalidArgumentError
...
def command_home(_: CLIContext) -> int:
    ...
    _emit(
        format_help(
            [
                "Run `pkgdx axi list` for the venv package list",
                "Run `pkgdx axi show <package>` for metadata information",
                "Run `pkgdx axi show <package> --api` for public API symbols",
                (
                    "Run `pkgdx axi find <query> --package <package>` to"
                    " resolve a bare symbol name"
                ),
                "Run `pkgdx axi tree <package>` for a nested module tree",
                "Run `pkgdx axi inspect <qualified_name>` for symbol detail",
                "Run `pkgdx axi inherits <qualified_name>` for subclasses",
                "Run `pkgdx axi serve` to start the MCP server over stdio",
                "Run `pkgdx axi setup` to install ambient context",
            ]
        )
    )
...
def command_list(ctx: CLIContext) -> int:
    ...
    if not packages:
        help_txt = "Run `pkgdx axi list --all` to include all dependencies"
    ...
    _emit(format_help(["Run `pkgdx axi show <package>` for package info"]))
    return ExitCode.EX_OK
    ...
    if not symbols:
        ...
        _emit(
            format_help(
                [
                    (
                        f"Run `pkgdx axi tree {ctx.args.package}`"
    ...
    if not ctx.args.docstring:
        _emit(
            format_help(
                [
                    (
                        f"Run `pkgdx axi show {ctx.args.package} "
...
def _command_show_metadata(ctx: CLIContext) -> int:
    ...
    _emit(
        format_help(
            [f"Run `pkgdx axi show {ctx.args.package} --api` for public API"]
        )
    )
    return ExitCode.EX_OK
            ...
            format_help(
                [
                    (
                        ...
                        " - run `pkgdx axi list --all` to check the"
                        " package name"
                    )
                    if package
                    else (
                        "Run `pkgdx axi find <query> --package <package>`"
...
def command_tree(ctx: CLIContext) -> int:
    ...
    if not pairs:
        _emit("count: 0")
        _emit(format_help(["Run `pkgdx axi list` for the venv package list"]))
        return ExitCode.EX_OK
    ...
    _emit(
        format_help(
            ["Run `pkgdx axi inspect <module>` for a module's symbols"]
        )
    )
    return ExitCode.EX_OK
            ...
            format_help(
                [
                    (
                        ...
                        " below the built depth - run `pkgdx axi find"
    ...
    if not children:
        ...
        _emit(
            format_help(
                [
                    (
                        f"Run `pkgdx axi tree {ctx.args.qualified_name}`"
                        " for the nested module tree"
                    )
                ]
            )
    ...
    if not docstring:
        _emit(
            format_help(
                [
                    (
                        f"Run `pkgdx axi inspect {ctx.args.qualified_name}"
...
def command_serve(_: CLIContext) -> int:
    ...
    if not mcp_available():
        logger.error("`pkgdx axi serve` requires the `pkgdx[axi]` extra")
...
def command_setup(_: CLIContext) -> int:
    ...
    hints = ["Run `pkgdx axi` to confirm ambient context is live"]
    if not mcp_available():
        hints.append("Run `uv add pkgdx[axi]` to enable the MCP server")
    _emit(format_help(hints))
    return ExitCode.EX_OK


def add_subparser(subparsers: "argparse._SubParsersAction[Any]") -> None:
    """Register the `axi` subcommand with the shared `pkgdx` CLI parser.

    Args:
        subparsers: The parent parser's subparsers action.
    """

    parser_axi = subparsers.add_parser(
        "axi",
        help="Agent eXperience Interface (AXI)",
    )

    parser_axi.set_defaults(func=command_home)

    axi_subparsers = parser_axi.add_subparsers(
        title="axi commands", dest="axi_command"
    )

    # `list` command
    parser_list = axi_subparsers.add_parser(
    ...
    # `show` command
    parser_show = axi_subparsers.add_parser(
    ...
    # `find` command
    parser_find = axi_subparsers.add_parser(
        "find",
    ...
    # `tree` command
    parser_tree = axi_subparsers.add_parser(
    ...
    # `inspect` command
    parser_inspect = axi_subparsers.add_parser(
        "inspect",
    ...
    # `inherits` command
    parser_inherits = axi_subparsers.add_parser(
        "inherits",
    ...
    # `serve` command
    parser_serve = axi_subparsers.add_parser(
        "serve",
        help="Run a dedicated AXI MCP server (requires pkgdx[axi])",
    )
    parser_serve.set_defaults(func=command_serve)

    # `setup` command
    parser_setup = axi_subparsers.add_parser(
        "setup",
        help=" ".join(
            [
