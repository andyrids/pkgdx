"""Lazy FastMCP server exposing `axi` data as MCP tools."""
...
from pkgdx.axi._packages import list_packages, resolve_package
...
from pkgdx.exceptions import AXIError

logger = logging.getLogger(__package__)


def _toon_errors(fn: Callable[..., str]) -> Callable[..., str]:
    """Wrap an MCP tool so `AXIError`s return a TOON error block.

    NOTE: Mirrors the CLI error contract - without this, `AXIError`s
...
def _with_help(output: str, hints: list[str]) -> str:
    ...
    surface too - `axi setup` registers the MCP server as the primary
...
def build_server() -> Any:
    """Build the `axi` FastMCP server instance.
