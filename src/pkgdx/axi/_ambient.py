...
NOTE: The above steps are idempotent - running `pkgdx axi setup` multiple
...
_BEGIN = "<!-- pkgdx:axi:begin -->"
_END = "<!-- pkgdx:axi:end -->"

# NOTE: Pre-rename (`venv-axi` -> `axi`) markers - migrated on setup
_OLD_BEGIN = "<!-- pkgdx:venv-axi:begin -->"
_OLD_END = "<!-- pkgdx:venv-axi:end -->"
...
def _axi_command() -> str:
    """Resolve absolute path of the `pkgdx` executable.

    Returns:
        The absolute path of the invoked `pkgdx` script.
    """
...
def mcp_available() -> bool:
    ...
    is safe on a plain `pkgdx` install (no `axi` extra).
...
def inject_agents_md(root: Path) -> bool:
    ...
    NOTE: A pre-rename `venv-axi` block is tool-owned content - it is
    sliced out first, so migrating repos neither keep a stale duplicate
    nor drift.
...
def _update_mcp_json(path: Path, servers_key: str, available: bool) -> bool:
    ...
    NOTE: A registered server that `pkgdx axi serve` cannot start would
    ...
    # NOTE: Pre-rename (`venv-axi`) server entries are dead - drop them
    stale = servers.pop("venv-axi", None) is not None

    if not available:
        removed = servers.pop("axi", None) is not None
        if not (stale or removed):
            return False
    ...
    if not stale and servers.get("axi") == entry:
        return False
