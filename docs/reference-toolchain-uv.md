...
When using `uv run` in this project, uv will install this package before running the script. If the
script does not depend on this package, use the `--no-project` option.
...
NOTE: Any dependencies not included with this project must be declared in the script.
