"""Unit tests for `pkgdx.axi._introspect`."""
...
from pkgdx.axi._store import NodeKind, SymbolStore
...
@pytest.fixture
def fake_package(
    isolated_axi_cache: Path, tmp_path_factory: pytest.TempPathFactory
) -> Iterator[str]:
...
def test_get_symbol_unimportable_raises(isolated_axi_cache: Path) -> None:
...
def test_find_symbol_blank_query_raises(isolated_axi_cache: Path) -> None:
...
def test_walk_submodules_skips_import_failure(
    ...
    with caplog.at_level(logging.WARNING, logger="pkgdx.axi"):
...
def test_resolve_import_name_returns_import_name_key_unchanged() -> None:
    ...
    with mock.patch(
        "pkgdx.axi._introspect.metadata.packages_distributions",
