"""Unit tests for `pkgdx.axi._cache`."""
...
from pkgdx.axi import _cache
from pkgdx.axi._store import NodeKind, SymbolNode, SymbolStore

NodeFactory = Callable[..., SymbolNode]

CACHE = "pkgdx.axi._cache"


@pytest.fixture
def fake_module(isolated_axi_cache: Path) -> Iterator[types.ModuleType]:
...
def test_get_cache_db_path_is_stable(
    ...
    assert first == second
    assert first.parent == isolated_axi_cache
...
def test_get_or_build_store_skips_rebuild_when_valid(
    ...
    with mock.patch(f"{CACHE}._installed_version", return_value="1.0.0"):
        ...
        first.close()
        with mock.patch("pkgdx.axi._introspect._walk_module") as walk:
...
def test_get_or_build_store_wraps_database_error(
    ...
    with (
        ...
        mock.patch(
            "pkgdx.axi._introspect._walk_module",
...
def test_get_or_build_store_import_failure_closes_store(
    tmp_path: Path, isolated_axi_cache: Path
) -> None:
