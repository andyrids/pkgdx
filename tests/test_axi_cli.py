...
from pkgdx.axi import _cli
from pkgdx.axi._introspect import SYMBOL_INFO_FIELDS, SymbolInfo
...
def _make_axi_parser() -> argparse.ArgumentParser:
    """Build a bare parser with the `axi` subcommand registered."""
    parser = argparse.ArgumentParser(prog="pkgdx")
...
def test_add_subparser_defaults_to_home() -> None:
    """A bare `axi` invocation dispatches to `command_home`."""
    parser = _make_axi_parser()
    args = parser.parse_args(["axi"])
    assert args.func is _cli.command_home


def test_add_subparser_list_defaults() -> None:
    """The `axi list` subcommand has the documented default flags."""
    parser = _make_axi_parser()
    args = parser.parse_args(["axi", "list"])
...
def test_add_subparser_show_requires_package() -> None:
    """The `axi show` subcommand requires a positional package name."""
    parser = _make_axi_parser()
    with pytest.raises(SystemExit):
        parser.parse_args(["axi", "show"])


def test_add_subparser_find_defaults() -> None:
    """The `axi find` subcommand has the documented default flags."""
    parser = _make_axi_parser()
    args = parser.parse_args(["axi", "find", "Console"])
...
def test_add_subparser_tree_defaults() -> None:
    """The `axi tree` subcommand has the documented default flags."""
    parser = _make_axi_parser()
    args = parser.parse_args(["axi", "tree", "rich"])
...
def test_add_subparser_inspect_requires_qualified_name() -> None:
    """The `axi inspect` subcommand requires a positional qualified name."""
    parser = _make_axi_parser()
    with pytest.raises(SystemExit):
        parser.parse_args(["axi", "inspect"])
    args = parser.parse_args(["axi", "inspect", "rich::Console"])
...
def test_command_serve_reports_missing_extra(
    ...
    with (
        ...
        mock.patch("pkgdx.axi._mcp.serve") as serve,
    ):
...
def test_command_serve_propagates_runtime_import_error(
    ...
    (previously: misreported as a missing `pkgdx[axi]` extra)."""
    with (
        mock.patch(f"{CLI}.mcp_available", return_value=True),
        mock.patch("pkgdx.axi._mcp.serve", side_effect=ImportError("boom")),
...
def test_add_subparser_inherits_requires_qualified_name() -> None:
    """The `axi inherits` subcommand requires a positional name."""
    parser = _make_axi_parser()
    with pytest.raises(SystemExit):
        parser.parse_args(["axi", "inherits"])
    args = parser.parse_args(["axi", "inherits", "rich::Console"])
...
def test_add_subparser_inherits_has_refresh() -> None:
    """The `axi inherits` subcommand accepts the `--refresh` flag."""
    parser = _make_axi_parser()
    args = parser.parse_args(["axi", "inherits", "x", "--refresh"])
...
def test_command_setup_hints_missing_extra(
    ...
    assert "uv add pkgdx[axi]" in out
...
def test_main_maps_axi_error_to_toon_and_exit_1(
    capsys: pytest.CaptureFixture,
) -> None:
    """An `AXIError` emits a TOON error block & maps to exit code 1."""
    error = exceptions.PackageNotFoundError("boom")
    with mock.patch(f"{CLI}.command_list", side_effect=error):
        exit_code = _run_main(["axi", "list"])
    out = capsys.readouterr().out
...
def test_main_maps_plain_error_to_exit_1_without_toon(
    capsys: pytest.CaptureFixture,
) -> None:
    """A non-AXI `Error` maps to exit code 1 with no TOON STDOUT output."""
    error = exceptions.ProjectRootNotFoundError("no root")
    with mock.patch("pkgdx.standards._cli.command_init", side_effect=error):
...
def test_main_maps_unexpected_error_to_exit_2() -> None:
    ...
    with mock.patch(f"{CLI}.command_home", side_effect=RuntimeError("oops")):
        exit_code = _run_main(["axi"])
    assert exit_code == ExitCode.EX_SYNTAX


def test_main_unexpected_axi_error_emits_toon_on_stdout(
    capsys: pytest.CaptureFixture,
) -> None:
    """An unexpected `axi` error keeps the TOON contract on stdout
    (no Rich traceback), with the traceback on stderr only."""
    with mock.patch(f"{CLI}.command_home", side_effect=RuntimeError("oops")):
        exit_code = _run_main(["axi"])
    out = capsys.readouterr().out
    assert exit_code == ExitCode.EX_SYNTAX
...
def test_main_verbose_flag_reaches_axi_context() -> None:
    """`pkgdx --verbose axi` parses & sets `CLIContext.is_verbose`."""
    ...
    with mock.patch(f"{CLI}.command_home", side_effect=record):
        exit_code = _run_main(["--verbose", "axi"])
...
def test_main_verbose_flag_reaches_init_context() -> None:
    ...
    with mock.patch("pkgdx.standards._cli.command_init", side_effect=record):
...
def test_main_defaults_to_non_verbose() -> None:
    """`pkgdx axi` without `--verbose` leaves `is_verbose` False."""
    recorded: dict[str, CLIContext] = {}
